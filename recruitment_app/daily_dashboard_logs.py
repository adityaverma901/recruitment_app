...
# Import the function directly from the same app
from recruitment_app.daily_dashboard_metric import get_daily_dashboard_data

def daily_api_data_sync():
    """
    Fetch data from daily dashboard metric and save to Daily Dashboard Logs DocType
    ...
    try:
        ...
        api_response = get_daily_dashboard_data()
        ...
        if not date or not metrics:
            frappe.log_error(
                ...
                message=f"Missing date or metrics in response: {json.dumps(api_response, indent=2)}"
            )
            return
