        ...
        query = """
            ...
            WHERE (
                ...
                OR EXISTS (
                    ...
                    WHERE ce.parent = c.name AND ce.email_id LIKE %s          #ce.email_id LIKE "%john%"    -- 2nd %s replaced  this just fetches wether mail is present or not it dont return the data actual
                )
                OR EXISTS (
                    SELECT 1 FROM `tabContact Phone` cp
                    WHERE cp.parent = c.name AND cp.phone LIKE %s              #cp.phone LIKE "%john%"    
                )
            )
